...
function crear_Click() {
    ...
    if (isAllValid) {
        var data = {
            MiembroMatriculaID: 0,
            MatriculasId: $('#MatriculasId').val().trim(),
        ...
        $.ajax({
            url: '/MiembroMatriculas/Create',
            type: "POST",
            data: { __RequestVerificationToken: token, miembromatricula: data },
            success: function (d) {
                if (d == true) {
